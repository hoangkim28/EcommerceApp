  ...
  <div class="min-h-screen py-12 sm:pt-20">
    <div
      ...
      <div class="w-full md:w-1/2 max-w-md border border-palette-lighter bg-white rounded shadow-lg">
        <div class="relative h-96 md:h-112">
          <div
            ...
            <img alt="test-text" src="{{voyager::image($product->image)}}" decoding="async" class="object-cover transform duration-500 ease-in-out hover:scale-105"
              sizes="100vw"
            ...
            <button class="relative w-40 h-32 flex-shrink-0 rounded-sm ">
              <div
                ...
                <img alt="test-text"
                  ...
                  decoding="async" class="" sizes="100vw"
      ...
      <div class="flex flex-col justify-between h-full w-full md:w-1/2 max-w-xs mx-auto space-y-4 min-h-128"><a
          ...
          href="{{str_replace(url('/'), '', url()->previous())}}"><svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="arrow-left"
            class="svg-inline--fa fa-arrow-left fa-w-14 w-4 mr-2 inline-flex" role="img"
        ...
        <div class=" font-primary">
          ...
          <div class="text-xl text-black font-medium py-4 px-1">
            <span class="price @if($promotion_price) line-through text-xl text-gray-500 @else text-2xl @endif ">
              {{number_format($price)}}đ</span>
              @if($promotion_price)
              <br>
              <span class="text-3xl text-palette-dark price">
        ...
        <div class="mt-4">
          ...
          <fieldset class="mt-4">
            ...
            <div class="grid grid-cols-4 gap-4 sm:grid-cols-5 lg:grid-cols-4">
              ...
              <label @if($sku->quantity !== 0)wire:click="selectSize({{$sku->size_id}})" @endif class="group relative border rounded-sm py-3 px-4 sm:py-5 flex items-center justify-center text-xl font-medium focus:outline-none sm:flex-1 hover:text-white font-primary 
              @if($sku->quantity === 0) cursor-not-allowed @else cursor-pointer hover:bg-black hover:border-black @endif
              @if($sku->size_id === $size_id) bg-black text-white @else bg-gray-50 text-black @endif
              ">
                  {{$sku->size->name}}
                @if($sku->quantity === 0)
                <div aria-hidden="true" class="absolute -inset-px rounded-md border-2 border-gray-400 pointer-events-none">
                  <svg class="absolute inset-0 w-full h-full text-gray-400 stroke-2" viewBox="0 0 100 100" preserveAspectRatio="none" stroke="currentColor">
        ...
        <div class="w-full">
          <div class="flex justify-start space-x-2 w-full">
            ...
            <h3 class="text-lg text-gray-900 font-primary">Số lượng</h3>
              <input wire:model="product_cart_quantity" type="number" inputmode="numeric" id="quantity"
          ...
          <button wire:click="addCart" class="pt-3 pb-2 bg-black text-white w-full mt-2 rounded-sm font-primary font-semibold text-xl flex justify-center items-baseline  hover:bg-palette-dark" aria-label="cart-button">
              Thêm giỏ hàng
    ...
    <div class="w-100 md:w-1/2 max-w-sm mx-14 mt-6 border md:mx-auto">
    @if($product->content)
