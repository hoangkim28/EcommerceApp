@if(isset($dataTypeContent->{$row->field}))
    <div data-field-name="{{ $row->field }}">
        ...
        <img class="show-single-image" src="@if( !filter_var($dataTypeContent->{$row->field}, FILTER_VALIDATE_URL)){{ Voyager::image( $dataTypeContent->{$row->field} ) }}@else{{ $dataTypeContent->{$row->field} }}@endif"
